...
class _Sunrise_Fields {
	...
	static function _fixup_fields() {
		...
		foreach( $fields as $field_index => $field_args ) {
			$fields[$field_index] = self::normalize_field_args( $fields[$field_index] );
		}
	 ...
	 * @return int
	 */
	static function normalize_field_args( $field_args ) {
		$field_args = wp_parse_args( $field_args );
		foreach( $field_args as $name => $value ) {
			$new_name = self::_maybe_prefix_field_arg( $name );
			if ( $new_name != $name ) {
				$field_args[$new_name] = $value;
	...
	/**
	 * Add a prefix to a field $arg if needed based on the property names of the class.
	 *
	 * @param string $field_name
	 * @return string
	 */
	private static function _maybe_prefix_field_arg( $field_name ) {
		return Sunrise::maybe_prefix_class_arg( $field_name, 'field_', 'Sunrise_Field_Base' );
	}
